...
def _normalize_equality_op(op: str) -> str:
    """Normalize ``==`` to ``=`` per taxonomy spec §7."""
...
def _normalize_filter_operator_shape(value: Any) -> List[str]:
    ...
    if not isinstance(operators, list):
        return [_normalize_equality_op(_to_schema_string(operators))]
    ...
    for item in operators:
        if isinstance(item, list):
            ...
            flat.append(item)
    return [_normalize_equality_op(s) for s in _stringify_list(flat or [""])]


def _normalize_nested_operator_shape(value: Any) -> List[List[str]]:
    if isinstance(value, str):
        return [[_normalize_equality_op(_to_schema_string(value))]]
    ...
    if all(not isinstance(item, list) for item in operators):
        return [[_normalize_equality_op(s) for s in _stringify_list(operators)]]

    normalized: List[List[str]] = []
    for item in operators:
        if isinstance(item, list):
            normalized.append([_normalize_equality_op(s) for s in _stringify_list(item or [""])])
        else:
            normalized.append([_normalize_equality_op(_to_schema_string(item))])
...
def _unwrap_envelope(payload: Any) -> Any:
    """Recursively unwrap double-nested envelopes generated by LLM."""
    if isinstance(payload, dict):
        if "taxonomy_rules" in payload and isinstance(payload["taxonomy_rules"], list):
            rules = payload["taxonomy_rules"]
            if len(rules) == 1 and isinstance(rules[0], dict) and ("taxonomy_rules" in rules[0] or "TaxonomyRules" in rules[0]):
                return _unwrap_envelope(rules[0])
        elif "TaxonomyRules" in payload and isinstance(payload["TaxonomyRules"], list):
            rules = payload["TaxonomyRules"]
            if len(rules) == 1 and isinstance(rules[0], dict) and ("taxonomy_rules" in rules[0] or "TaxonomyRules" in rules[0]):
                return _unwrap_envelope(rules[0])
    return payload
